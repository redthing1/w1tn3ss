...
Example CMakeLists.txt:
    ...
    set(WITNESS_BUILD_ALL OFF CACHE BOOL "" FORCE)
    set(WITNESS_BUILD_W1REWIND ON CACHE BOOL "" FORCE)
...
Example:
    ...
    set(WITNESS_BUILD_ALL OFF CACHE BOOL "" FORCE)
    set(WITNESS_BUILD_W1REWIND ON CACHE BOOL "" FORCE)
    set(WITNESS_BUILD_W1REPLAY ON CACHE BOOL "" FORCE)
    add_subdirectory(path/to/w1tn3ss w1tn3ss_build)

Component options are named WITNESS_BUILD_<COMPONENT>, where <COMPONENT> matches the directory name (uppercased). For example, WITNESS_BUILD_W1REWIND, WITNESS_BUILD_W1RUNTIME, WITNESS_BUILD_TRACERS, and WITNESS_BUILD_P1LL.

## Notes

- Heavy dependencies like QBDI and LIEF are only configured when a component that needs them is enabled.
- Use WITNESS_USE_SYSTEM_DEPS=ON to prefer system-installed dependencies via find_package.
