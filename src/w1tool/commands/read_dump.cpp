...
int read_dump(
  ...
  try {
    ...
    // always show the metadata overview
    {
      ...
      std::cout << "│  ├─ Platform:  " << dump.metadata.os << "/" << dump.metadata.arch << " (" << static_cast<int>(dump.metadata.pointer_size) * 8 << "-bit)\n";
      ...
      for (const auto& mod : dump.modules) {
        if (mod.is_system_library) system_modules++;
        else user_modules++;
      }
      ...
      std::cout << "   ├─ Registers: " << dump.thread.gpr_values.size() << " GPR, " << dump.thread.fpr_values.size() << " FPR\n";
    ...
    if (detailed_flag) {
      ...
      auto print_module_line = [](const module_info_serializable* mod, bool show_system) {
        std::cout << std::hex << std::setw(12) << mod->base_address << std::dec 
                  << " " << std::setw(8) << std::right << format_bytes_dump(mod->size)
                  << " " << std::setw(3) << format_permissions(mod->permissions)
                  << " " << std::left << std::setw(32) << mod->name;
        if (show_system) std::cout << " [sys]";
        std::cout << "\n";
      };
      ...
      for (const auto& region : dump.regions) {
        if (region.permissions == 0) continue; // skip unmapped
      ...
      auto print_region_group = [](const RegionGroup& group) {
        if (group.regions.empty()) return;
        
        std::cout << "── " << group.name << " (" << group.regions.size() << " regions, " 
                  << format_bytes_dump(group.total_size) << ") ";
        for (int i = 0; i < 70 - group.name.length() - 20; i++) std::cout << "─";
        std::cout << "\n";
        
        for (const auto* region : group.regions) {
          std::cout << std::hex << std::setw(12) << region->start 
                    << " " << std::setw(12) << region->end << std::dec
                    << " " << std::setw(8) << format_bytes_dump(region->end - region->start)
                    << " " << std::setw(3) << format_permissions(region->permissions)
                    << " " << std::setw(4) << (region->is_stack ? "STK" : 
                                               region->is_code ? "CODE" : "DATA");
    ...
    log.err("failed to read dump", 
           redlog::field("file", file_path),
