...
int cover(args::ValueFlag<std::string>& binary_flag,
    ...
    int target_count = 0;
    if (binary_flag) target_count++;
    if (pid_flag) target_count++;
    if (name_flag) target_count++;
    ...
    if (format_flag) {
        ...
        if (format != "drcov" && format != "text") {
            log.error("invalid format, supported: drcov, text", 
                     redlog::field("format", format));
            return 1;
        }
    }
    ...
    log.info("coverage tracing configuration",
    ...
    if (binary_flag) {
        ...
        std::string binary_path = args::get(binary_flag);
        log.info("starting launch-time coverage tracing",
        ...
        log.info("starting runtime coverage tracing",
    ...
    // Handle result
    if (result.success()) {
        log.info("coverage tracing completed successfully",
                 redlog::field("output_file", output_file));
        ...
        log.error("coverage tracing failed",
