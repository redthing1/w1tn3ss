...
void symbol_enricher::initialize(const w1::util::module_range_index& module_index) {
    auto log = redlog::get_logger("w1xfer::symbol_enricher");
    log.dbg("initializing symbol enricher with module index",
            redlog::field("module_count", module_index.size()));
    module_index_ = &module_index;
}

std::optional<symbol_enricher::symbol_context> 
symbol_enricher::enrich_address(uint64_t address) const {
#ifdef WITNESS_LIEF_ENABLED
    auto log = redlog::get_logger("w1xfer::symbol_enricher");
    ...
    if (!module) {
        log.dbg("no module found for address", 
    ...
    log.dbg("resolving symbol", 
             redlog::field("address", address),
             redlog::field("module_name", module->name),
             redlog::field("module_path", module->path),
             redlog::field("module_offset", module_offset));
    ...
    log.trc("symbol resolved", 
            redlog::field("address", address),
            redlog::field("symbol_name", symbol->name),
            redlog::field("demangled_name", symbol->demangled_name),
            redlog::field("symbol_offset", symbol->offset),
...
std::vector<std::optional<symbol_enricher::symbol_context>> 
symbol_enricher::enrich_addresses(const std::vector<uint64_t>& addresses) const {
#ifdef WITNESS_LIEF_ENABLED
    if (!resolver_ || !module_index_) {
