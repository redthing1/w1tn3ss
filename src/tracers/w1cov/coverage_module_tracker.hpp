...
public:
    ...
    bool visit_traced_module(QBDI::rword address, Visitor&& visitor) const;
     ...
     */
    template<typename Visitor>
    bool try_rescan_and_visit(QBDI::rword address, Visitor&& visitor);
...
bool coverage_module_tracker::visit_traced_module(QBDI::rword address, Visitor&& visitor) const {
    static_assert(std::is_invocable_v<Visitor, const w1::util::module_info&, uint16_t>, 
...
template<typename Visitor>
bool coverage_module_tracker::try_rescan_and_visit(QBDI::rword address, Visitor&& visitor) {
    static_assert(std::is_invocable_v<Visitor, const w1::util::module_info&, uint16_t>, 
                  "visitor must be callable with (const module_info&, uint16_t)");
    ...
    if (!rescan_lock.owns_lock()) {
        log_.dbg("rescan already in progress, skipping", 
                 redlog::field("address", "0x%08x", address));
        return false;
    }
    ...
    try {
        ...
        if (modules_to_add.empty()) {
            log_.dbg("no new traced modules after filtering", 
                     redlog::field("discovered_modules", new_modules.size()));
            return false;
        }
        ...
        for (const auto& mod : modules_to_add) {
            ...
            log_.dbg("added new traced module", 
        ...
        log_.inf("rescan completed successfully", 
                 redlog::field("new_traced_modules", modules_to_add.size()),
