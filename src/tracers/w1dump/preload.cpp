...
static std::unique_ptr<w1::trace_session<w1dump::dump_tracer>> g_session;
...
QBDI_EXPORT int qbdipreload_on_run(QBDI::VMInstanceRef vm, QBDI::rword start, QBDI::rword stop) {
  ...
  g_config = w1dump::dump_config::from_environment();
  ...
  g_session = std::make_unique<w1::trace_session<w1dump::dump_tracer>>(session_config, vm, std::in_place, g_config);
  ...
  bool run_ok = g_session->run(static_cast<uint64_t>(start), static_cast<uint64_t>(stop));
  if (!run_ok) {
    if (g_session->tracer().dump_completed()) {
