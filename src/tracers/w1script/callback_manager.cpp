...
void callback_manager::setup_callbacks(const sol::table& script_table) {
    ...
    logger_.inf("callbacks setup complete", 
        redlog::field("enabled", enabled_callbacks_.size()),
...
void callback_manager::register_callbacks(QBDI::VM* vm) {
    ...
    for (const auto& [cb_type, event] : vm_events) {
        if (is_callback_enabled(cb_type)) {
            uint32_t id = vm->addVMEventCB(
                event,
                [this, cb_type](
                    QBDI::VMInstanceRef vm, const QBDI::VMState* state, 
                    QBDI::GPRState* gpr, QBDI::FPRState* fpr
                ) -> QBDI::VMAction {
                    return this->dispatch_vm_event_callback(cb_type, vm, state, gpr, fpr);
                }
            );
            if (id != QBDI::INVALID_EVENTID) {
                registered_callback_ids_.push_back(id);
                logger_.inf("registered vm event callback", 
                    redlog::field("type", static_cast<int>(cb_type)), 
                    redlog::field("id", id)
                );
            }
        }
    ...
    for (const auto& [cb_type, access_type] : memory_accesses) {
        if (is_callback_enabled(cb_type)) {
            ...
            if (id != QBDI::INVALID_EVENTID) {
                registered_callback_ids_.push_back(id);
                logger_.inf("registered memory access callback",
                    redlog::field("type", static_cast<int>(cb_type)),
                    redlog::field("id", id)
                );
            }
    ...
    logger_.inf("dynamic callback registration complete", 
        redlog::field("total_callbacks", registered_callback_ids_.size())
    );
}

QBDI::VMAction callback_manager::dispatch_simple_callback(
    callback_type type, QBDI::VMInstanceRef vm, 
    QBDI::GPRState* gpr, QBDI::FPRState* fpr
) {
...
QBDI::VMAction callback_manager::dispatch_vm_event_callback(
    ...
    try {
        ...
        logger_.err("error in vm event callback",
            redlog::field("type", static_cast<int>(type)),
            redlog::field("error", e.what())
        );
    }
...
std::vector<QBDI::InstrRuleDataCBK> callback_manager::dispatch_instr_rule_callback(
    callback_type type, QBDI::VMInstanceRef vm,
    const QBDI::InstAnalysis* analysis, void* data
) {
    auto it = lua_callbacks_.find(type);
    if (it == lua_callbacks_.end() || !it->second.valid()) {
        return {};
    ...
    try {
        ...
        logger_.err("error in instr rule callback",
            redlog::field("type", static_cast<int>(type)),
            redlog::field("error", e.what())
        );
    }
