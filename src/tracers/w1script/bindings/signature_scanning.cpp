...
std::shared_ptr<p1ll::context> acquire_p1_context() {
  static std::shared_ptr<p1ll::context> context_holder = []() {
    auto context = p1ll::context::create_dynamic();
    return std::shared_ptr<p1ll::context>(std::move(context));
  }();
...
void setup_signature_scanning(sol::state& lua, sol::table& w1_module) {
  ...
  auto context = acquire_p1_context();
  if (!context) {
    logger.err("failed to acquire p1ll context for lua bindings");
    return;
  }
