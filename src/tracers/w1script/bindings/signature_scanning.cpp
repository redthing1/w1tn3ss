...
void setup_signature_scanning(sol::state& lua, sol::table& w1_module) {
  ...
  w1_module.set_function("hook_sig", [&logger, &lua, &w1_module](sol::object sig_obj, sol::protected_function handler) -> sol::optional<uint32_t> {
    try {
  ...
  w1_module.set_function("hook_all_sigs", [&logger, &lua, &w1_module](sol::object sig_obj, sol::protected_function handler) -> sol::optional<sol::table> {
    try {
      void* hook_mgr_ptr = w1_module["_hook_manager"];
      auto hook_mgr = static_cast<w1::hooking::hook_manager*>(hook_mgr_ptr);
