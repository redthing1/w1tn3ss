...
void setup_output(sol::state& lua, sol::table& w1_module) {
    ...
    // bind init function
    output_table["init"] = [output_instance, &lua](const std::string& filename, sol::optional<sol::table> metadata) -> bool {
    ...
    // bind close function
    output_table["close"] = [output_instance]() {
        output_instance->close();
    };
    
    // bind status functions
    output_table["is_initialized"] = [output_instance]() -> bool {
        return output_instance->is_initialized();
    };
    
    output_table["get_event_count"] = [output_instance]() -> size_t {
        return output_instance->get_event_count();
    };
