...
error_code validate_config(const config& cfg) {
    ...
    log.trace("target specification check",
              redlog::field("targets", target_count),
    ...
    if (target_count != 1) {
        log.error("invalid target specification - exactly one target required",
                 redlog::field("targets", target_count));
    ...
    if (!std::filesystem::exists(cfg.library_path)) {
        log.error("injection library not found",
    ...
    if (fs_error) {
        log.error("failed to get library file status",
                 redlog::field("library_path", cfg.library_path),
    ...
    if (!std::filesystem::is_regular_file(file_status)) {
        log.error("library path is not a regular file",
                 redlog::field("library_path", cfg.library_path));
        return error_code::library_not_found;
    }
    
    log.verbose("library validation passed",
               redlog::field("library_path", cfg.library_path),
    ...
    log.debug("validating method compatibility",
              redlog::field("method", method_str));
    
    if (cfg.injection_method == method::launch && !cfg.binary_path) {
        log.error("launch injection method requires binary_path",
                 redlog::field("method", method_str));
        return error_code::configuration_invalid;
    }
    
    if (cfg.injection_method == method::runtime && cfg.binary_path) {
        log.error("runtime injection method incompatible with binary_path",
                 redlog::field("method", method_str));
        return error_code::configuration_invalid;
    }
    ...
    log.debug("performing platform-specific validation",
              redlog::field("platform", platform_str),
              redlog::field("method", method_str));
    
#ifdef _WIN32
    if (cfg.injection_method == method::launch) {
        log.error("launch injection method not supported on windows",
                 redlog::field("platform", platform_str),
                 redlog::field("method", method_str));
...
result inject(const config& cfg) {
    ...
    log.info("injection request received",
             redlog::field("method", method_str),
             redlog::field("platform", platform_str),
             redlog::field("library_path", cfg.library_path));
    ...
    if (!cfg.env_vars.empty()) {
        log.verbose("environment variables configured",
                   redlog::field("vars", cfg.env_vars.size()));
        for (const auto& [key, value] : cfg.env_vars) {
            log.trace("environment variable",
    ...
    if (validation_result != error_code::success) {
        ...
        log.error("configuration validation failed",
                 redlog::field("error_code", static_cast<int>(validation_result)),
                 redlog::field("detail", error_detail),
                 redlog::field("method", method_str),
                 redlog::field("library_path", cfg.library_path));
    ...
    log.debug("checking injection capabilities for platform",
              redlog::field("platform", platform_str));
    
    bool capabilities_ok = check_injection_capabilities();
    if (!capabilities_ok) {
        log.warn("injection capabilities limited on this platform",
                redlog::field("platform", platform_str),
                redlog::field("method", method_str));
    }
    
    // 3. platform detection and dispatch
    log.debug("dispatching to platform-specific injection implementation",
              redlog::field("platform", platform_str),
              redlog::field("method", method_str));
...
#else
    log.error("platform not supported for injection",
             redlog::field("platform", platform_str));
    injection_result = make_error_result(error_code::platform_not_supported, 
                                       "injection not supported on platform: " + std::string(platform_str));
#endif
    ...
    if (injection_result.success()) {
        log.info("injection completed successfully",
                redlog::field("method", method_str),
                redlog::field("platform", platform_str),
                redlog::field("pid", injection_result.target_pid),
                redlog::field("library_path", cfg.library_path));
    } else {
