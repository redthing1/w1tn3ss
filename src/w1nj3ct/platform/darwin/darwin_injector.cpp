...
result inject_runtime(const config& cfg) {
    ...
    log.info("darwin runtime injection starting",
    ...
    if (cfg.process_name) {
        log.debug("resolving process name to pid",
                  redlog::field("name", *cfg.process_name));
        
        auto processes = find_processes_by_name(*cfg.process_name);
        if (processes.empty()) {
            log.error("no processes found with specified name",
                     redlog::field("name", *cfg.process_name));
            return make_error_result(error_code::target_not_found, *cfg.process_name);
        }
        if (processes.size() > 1) {
            log.error("multiple processes found with specified name",
                     redlog::field("name", *cfg.process_name),
                     redlog::field("count", processes.size()));
            for (const auto& proc : processes) {
                log.verbose("found process",
                           redlog::field("pid", proc.pid),
                           redlog::field("name", proc.name),
        ...
        log.info("resolved process name to pid",
                 redlog::field("name", *cfg.process_name),
                 redlog::field("pid", target_pid),
                 redlog::field("path", processes[0].full_path));
    } else {
        target_pid = *cfg.pid;
        log.debug("using specified pid",
                  redlog::field("pid", target_pid));
        
        // validate target process exists
        auto proc_info = get_process_info(target_pid);
        if (proc_info) {
            log.verbose("target process validated",
                       redlog::field("pid", target_pid),
                       redlog::field("name", proc_info->name),
                       redlog::field("path", proc_info->full_path));
        } else {
            log.warn("target process info not available",
                    redlog::field("pid", target_pid));
        }
    }
    ...
    log.debug("attaching to target process",
              redlog::field("pid", target_pid));
    ...
    if (err != INJERR_SUCCESS) {
        ...
        switch (err) {
            case INJERR_NO_PROCESS:
                ...
                log.error("target process not found during attach",
                         redlog::field("pid", target_pid),
                         redlog::field("injector_error", error_detail),
                         redlog::field("error_code", err));
                break;
            case INJERR_PERMISSION:
                mapped_error = error_code::target_access_denied;
                log.error("permission denied attaching to target process",
                         redlog::field("pid", target_pid),
                         redlog::field("injector_error", error_detail),
                         redlog::field("error_code", err),
                         redlog::field("uid", getuid()),
                         redlog::field("euid", geteuid()));
                break;
            case INJERR_NO_MEMORY:
                mapped_error = error_code::out_of_memory;
                log.error("out of memory during process attach",
                         redlog::field("pid", target_pid),
                         redlog::field("injector_error", error_detail),
                         redlog::field("error_code", err));
                break;
            default:
                ...
                log.error("unknown error during process attach",
                         redlog::field("pid", target_pid),
                         redlog::field("injector_error", error_detail),
                         redlog::field("error_code", err));
                break;
    ...
    log.info("successfully attached to target process",
             redlog::field("pid", target_pid),
             redlog::field("attach_time_ms", attach_ms));
    
    // inject the library
    auto inject_start = std::chrono::steady_clock::now();
    log.debug("injecting library into target process",
              redlog::field("pid", target_pid),
              redlog::field("library_path", cfg.library_path));
    ...
    log.trace("detached from target process",
              redlog::field("pid", target_pid));
    ...
    if (err != INJERR_SUCCESS) {
        ...
        switch (err) {
            case INJERR_FILE_NOT_FOUND:
                ...
                log.error("library file not found during injection",
                         redlog::field("pid", target_pid),
                         redlog::field("library_path", cfg.library_path),
                         redlog::field("injector_error", error_detail),
                         redlog::field("error_code", err));
            ...
            case INJERR_NO_MEMORY:
                mapped_error = error_code::out_of_memory;
                log.error("out of memory during library injection",
                         redlog::field("pid", target_pid),
                         redlog::field("library_path", cfg.library_path),
                         redlog::field("injector_error", error_detail),
                         redlog::field("error_code", err));
            ...
            case INJERR_ERROR_IN_TARGET:
                mapped_error = error_code::injection_failed;
                log.error("error in target process during injection",
                         redlog::field("pid", target_pid),
                         redlog::field("library_path", cfg.library_path),
                         redlog::field("injector_error", error_detail),
                         redlog::field("error_code", err));
            ...
            case INJERR_PERMISSION:
                mapped_error = error_code::target_access_denied;
                log.error("permission denied during library injection",
                         redlog::field("pid", target_pid),
                         redlog::field("library_path", cfg.library_path),
                         redlog::field("injector_error", error_detail),
                         redlog::field("error_code", err));
            ...
            default:
                mapped_error = error_code::injection_failed;
                log.error("unknown error during library injection",
                         redlog::field("pid", target_pid),
                         redlog::field("library_path", cfg.library_path),
                         redlog::field("injector_error", error_detail),
                         redlog::field("error_code", err));
    ...
    auto total_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - attach_start).count();
...
result inject_preload(const config& cfg) {
    ...
    log.info("darwin preload injection starting",
    ...
    log.debug("target binary validated",
              redlog::field("binary_path", *cfg.binary_path));
    ...
    log.trace("inherited environment variables",
              redlog::field("count", base_env_count));
    
    // add/override with cfg.env_vars
    for (const auto& [key, value] : cfg.env_vars) {
        env[key] = value;
        log.verbose("adding environment variable",
                   redlog::field("key", key),
                   redlog::field("value", value));
    }
    
    // Add DYLD_INSERT_LIBRARIES
    env["DYLD_INSERT_LIBRARIES"] = cfg.library_path;
    log.info("configured DYLD_INSERT_LIBRARIES",
             redlog::field("library_path", cfg.library_path));
    
    // warn about potential SIP issues
    if (cfg.binary_path->find("/System/") == 0 || 
        cfg.binary_path->find("/usr/bin/") == 0 ||
        cfg.binary_path->find("/bin/") == 0) {
        log.warn("injecting into system binary may fail due to SIP",
                redlog::field("binary_path", *cfg.binary_path));
    }
    ...
    for (const auto& arg : cfg.args) {
        ...
        log.trace("adding command argument",
                 redlog::field("arg", arg));
    }
    argv.push_back(nullptr);
    
    log.debug("command line prepared",
              redlog::field("argc", argv.size() - 1));
    ...
    log.debug("environment prepared",
              redlog::field("vars", env.size()),
              redlog::field("total_vars", envp.size() - 1));
    ...
    if (child_pid == 0) {
        // child process
        log.trace("child process executing target binary",
                 redlog::field("binary_path", *cfg.binary_path));
        ...
        log.info("preload injection completed successfully",
                 redlog::field("pid", child_pid),
                 redlog::field("binary_path", *cfg.binary_path),
                 redlog::field("library_path", cfg.library_path));
        ...
        log.error("fork failed during preload injection",
                 redlog::field("binary_path", *cfg.binary_path),
                 redlog::field("errno", fork_errno),
                 redlog::field("error", strerror(fork_errno)));
...
std::vector<process_info> list_processes() {
    ...
    log.debug("process list retrieved",
              redlog::field("total_processes", processes.size()),
...
std::vector<process_info> find_processes_by_name(const std::string& name) {
    ...
    log.debug("searching for processes by name",
              redlog::field("target_name", name));
    ...
    for (const auto& proc : all_processes) {
        if (proc.name == name) {
            ...
            log.verbose("found matching process",
                       redlog::field("pid", proc.pid),
                       redlog::field("name", proc.name),
                       redlog::field("path", proc.full_path));
        }
    ...
    log.debug("process search completed",
              redlog::field("target_name", name),
              redlog::field("matches_found", matches.size()),
...
std::optional<process_info> get_process_info(int pid) {
    ...
    log.trace("looking up process info",
              redlog::field("pid", pid));
    
    auto all_processes = list_processes();
    
    for (const auto& proc : all_processes) {
        if (proc.pid == pid) {
            log.trace("found process info",
                     redlog::field("pid", proc.pid),
                     redlog::field("name", proc.name),
                     redlog::field("path", proc.full_path));
            return proc;
    ...
    log.debug("process not found",
              redlog::field("pid", pid),
              redlog::field("searched", all_processes.size()));
...
bool check_injection_capabilities() {
    ...
    log.trace("attempting self-injection test",
              redlog::field("pid", current_pid));
    ...
    if (err == INJERR_SUCCESS) {
        ...
        log.warn("injection capabilities limited - self-attach failed",
                redlog::field("error_code", err),
                redlog::field("injector_error", injector_error()),
                redlog::field("uid", getuid()),
                redlog::field("euid", geteuid()));
        return false;
    }
