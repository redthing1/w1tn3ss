...
// https://raw.githubusercontent.com/gregjesl/simpleson/master/json.h
...
public:
   ...
   * \returns `ACCEPTED` if the value was added to the reader, `WHITESPACE` if the input was whitespace that was not
   * stored, and `REJECTED` is the input was invalid for the value type
   */
...
protected:
   ...
   *
   * This template is intended for use with #string_reader_enum, #number_reader_enum, #array_reader_enum, and
   * #object_reader_enum
   */
  template <typename T> T get_state() const { return static_cast<T>(this->read_state); }
   ...
   * #object_reader_enum
   */
  template <typename T> void set_state(const T state) { this->read_state = (char) state; }

  /*! \brief Enumeration of the state machine for strings */
  enum string_reader_enum {
    STRING_EMPTY = 0,     ///< No values have been read
    STRING_OPENING_QUOTE, ///< The opening quote has been read. Equivalant to #STRING_OPEN, but used for debugging the
                          ///< state
    STRING_OPEN,          ///< The opening quote has been read and the last character was not an escape character
    STRING_ESCAPED,       ///< The last character was an reverse solidus (\), indicating the next character should be a
                          ///< control character
    STRING_CODE_POINT_START, ///< An encoded unicode character is encountered. Expecting four following hex digits.
    STRING_CODE_POINT_1, ///< An encoded unicode character is encountered. Expecting three following hex digits (one has
                         ///< already been read).
    STRING_CODE_POINT_2, ///< An encoded unicode character is encountered. Expecting two following hex digits (two have
                         ///< already been read).
    STRING_CODE_POINT_3, ///< An encoded unicode character is encountered. Expecting one following hex digit (three has
                         ///< already been read).
  ...
  enum object_reader_enum {
    ...
    OBJECT_AWAITING_NEXT_LINE, ///< An object key value pair has been read and a comma was encountered. Expecting new
                               ///< line.
...
public:
  /*! \brief Constructor */
  inline kvp_reader() : reader() { this->clear(); }
   ...
   */
  inline virtual bool is_valid() const { return reader::is_valid() && this->_key.is_valid(); }
 ...
 *
 * \details Given a string, returns a pointer to the first character that is not white space. Spaces, tabs, and carriage
 * returns are all considered white space.
 ...
 *
 * \details This function will take an input string and read the digits at the front of the string until a character
 * other than a digit (0-9) is encountered.
 ...
 *
 * \details The quotation mark ("), reverse solidus (\), solidus (/), backspace (b), formfeed (f), linefeed (n),
 * carriage return (r), horizontal tab (t), and Unicode character will be unescaped
 ...
 *
 * \details The quotation mark ("), reverse solidus (\), solidus (/), backspace (b), formfeed (f), linefeed (n),
 * carriage return (r), horizontal tab (t), and Unicode character will be escaped
 ...
 */
template <typename T> std::string get_number_string(const T& number, const char* format) {
...
public:
   ...
   */
  inline jobject(bool array = false) : array_flag(array) {}

  /*! \brief Copy constructor */
  inline jobject(const jobject& other) : data(other.data), array_flag(other.array_flag) {}
   ...
   *
   * \exception json::parsing_error Thrown if the key-value is incompatable with the existing object (object/array
   * mismatch)
   */
   ...
   *
   * @return A list of keys contained in the object. If the object is actionally an array, an empty list will be
   * returned
   */
   ...
   *
   * \details If the key exists, then the value is updated. If the key does not exist, then the key value pair is added
   * to the object.
   ...
   */
  inline std::string get(const size_t index) const { return this->data.at(index).second; }
   ...
   * \exception json::invalid_key Exception thrown if the key does not exist in the object or the object actually
   ...
   */
  void remove(const size_t index) { this->data.erase(this->data.begin() + index); }
  ...
  protected:
     ...
     */
    template <typename T> inline T get_number(const char* format) const {
     ...
     */
    template <typename T> inline std::vector<T> get_number_array(const char* format) const {
  ...
  public:
     ...
     */
    inline json::jobject as_object() const { return json::jobject::parse(this->ref().c_str()); }

    /*! \see json::jobject::entry::as_object() */
    inline operator json::jobject() const { return this->as_object(); }
     ...
     */
    template <typename T> inline std::vector<T> as_array() const { return (std::vector<T>) (*this); }

    /*! \brief Returns true if the value is a string */
    inline bool is_string() const { return json::parsing::parse(this->ref().c_str()).type == json::jtype::jstring; }

    /*! \brief Returns true if the value is a number */
    inline bool is_number() const { return json::parsing::parse(this->ref().c_str()).type == json::jtype::jnumber; }
    ...
    inline bool is_array() const { return json::parsing::parse(this->ref().c_str()).type == json::jtype::jarray; }

    /*! \brief Returns true if the value is a bool */
    inline bool is_bool() const { return json::parsing::parse(this->ref().c_str()).type == json::jtype::jbool; }
    ...
    inline bool is_null() const { return json::parsing::parse(this->ref().c_str()).type == json::jtype::jnull; }
  };
  ...
  protected:
     ...
     */
    inline const std::string& ref() const { return this->data; }
  ...
  public:
     ...
     */
    inline const_value(std::string value) : data(value) {}
  ...
  protected:
     ...
     */
    template <typename T> inline void set_number(const T value, const char* format) {
     ...
     */
    template <typename T> inline void set_number_array(const std::vector<T>& values, const char* format) {
  ...
  public:
     ...
     */
    proxy(jobject& source, const std::string key) : json::jobject::const_proxy(source, key), sink(source) {}
    ...
    /*! \brief Assigns a string value */
    inline void operator=(const char* value) { this->operator=(std::string(value)); }
    ...
    void operator=(json::jobject input) { this->sink.set(key, (std::string) input); }
    ...
    /*! Sets a null value */
    inline void set_null() { this->sink.set(key, "null"); }

    /*! Clears the value */
    inline void clear() { this->sink.remove(key); }
  };
   ...
   * \note The serialized object or array will be in the most compact form and will not contain any extra white space,
   * even if the serialized string used to generate the object or array contained extra white space.
   */
  inline std::string as_string() const { return this->operator std::string(); }
