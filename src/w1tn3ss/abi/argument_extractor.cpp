...
public:
    impl(std::shared_ptr<api_knowledge_db> api_db,
         std::shared_ptr<calling_convention_detector> detector,
         const extractor_config& config)
        : api_db_(api_db), detector_(detector), config_(config),
    ...
    impl(std::shared_ptr<calling_convention_base> convention,
         std::shared_ptr<api_knowledge_db> api_db,
         const extractor_config& config)
        : default_convention_(convention), api_db_(api_db), config_(config),
          log_("w1::abi::argument_extractor") {
    ...
    extracted_call_info extract_call_with_convention(
        ...
        if (api_info) {
            ...
            for (size_t i = 0; i < raw_args.size(); ++i) {
                if (raw_args[i] == 0 && i > 2) break;  // heuristic
    ...
    void extract_return_value(
        ...
        if (api_info && api_info->return_value.param_type != param_info::type::VOID) {
            call_info.return_value = extract_argument(
                api_info->return_value, ret_val, memory);
        } else {
    ...
    extracted_argument extract_argument(
        ...
        if (param.param_type == param_info::type::POINTER ||
            param.param_type == param_info::type::STRING ||
        ...
        switch (param.param_type) {
            ...
            case param_info::type::BUFFER:
                if (!arg.is_null_pointer && config_.follow_pointers) {
                    // read buffer preview
                    auto buffer_result = memory.read_buffer(raw_value, 
                                                           std::min(config_.max_buffer_preview, size_t(256)));
                    if (buffer_result) {
    ...
    std::string format_call(const extracted_call_info& call) const {
        ...
        if (call.return_value.raw_value != 0 || 
            call.return_value.param_type != param_info::type::UNKNOWN) {
