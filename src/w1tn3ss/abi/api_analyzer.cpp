...
public:
    ...
    void initialize(const util::module_range_index& module_index) {
        ...
        log_.dbg("initialized with module index", 
                 redlog::field("module_count", module_index.size()));
    }
    ...
    api_analysis_result analyze_call(const api_context& ctx) {
        ...
        try {
            ...
            if (!result.symbol_name.empty()) {
                log_.dbg("looking up api in knowledge db", 
                         redlog::field("symbol", result.symbol_name));
                ...
                if (auto api_info = api_db_->lookup(result.symbol_name)) {
                    ...
                    log_.dbg("found api in knowledge db",
                             redlog::field("symbol", result.symbol_name),
                    ...
                    if (config_.extract_arguments && !api_info->parameters.empty()) {
                        log_.dbg("extracting arguments",
                                 redlog::field("symbol", result.symbol_name),
                                 redlog::field("param_count", api_info->parameters.size()));
                        extract_arguments(result, ctx, *api_info);
                    } else if (config_.extract_arguments && api_info->parameters.empty()) {
                        log_.dbg("no parameters defined for api",
                                 redlog::field("symbol", result.symbol_name));
                    }
                } else {
                    log_.dbg("api not found in knowledge db",
                             redlog::field("symbol", result.symbol_name));
                }
            }
    ...
    void analyze_return(api_analysis_result& result, const api_context& ctx) {
        ...
        try {
            ...
            if (auto api_info = api_db_->lookup(result.symbol_name)) {
                ...
                result.return_value = argument_extractor_.extract_argument(
                    api_info->return_value,
                    ret_val,
                    util::safe_memory_reader{ctx.vm}
                );
            }
        } catch (const std::exception& e) {
...
private:
    void resolve_symbol(api_analysis_result& result, uint64_t address, const util::module_info& module) {
        ...
        if (auto symbol = symbol_resolver_->resolve_in_module(module.path, result.module_offset)) {
            ...
            log_.dbg("resolved symbol",
                     redlog::field("address", address),
                     redlog::field("symbol", symbol->name),
    ...
    void extract_arguments(api_analysis_result& result, const api_context& ctx, const api_info& api) {
        ...
        for (size_t i = 0; i < raw_args.size() && i < api.parameters.size(); ++i) {
            log_.dbg("extracting argument",
                     redlog::field("index", i),
                     redlog::field("param_name", api.parameters[i].name),
                     redlog::field("raw_value", raw_args[i]),
                     redlog::field("type", static_cast<int>(api.parameters[i].param_type)));
            
            auto arg = argument_extractor_.extract_argument(
                api.parameters[i],
                raw_args[i],
                memory
            );
            
            log_.dbg("extracted argument",
                     redlog::field("index", i),
                     redlog::field("param_name", arg.param_name),
                     redlog::field("raw_value", arg.raw_value),
                     redlog::field("string_preview", arg.string_preview),
        ...
        log_.dbg("argument extraction complete",
                 redlog::field("extracted_count", result.arguments.size()));
    }
    ...
    void format_call(api_analysis_result& result) {
        ...
        for (size_t i = 0; i < result.arguments.size(); ++i) {
            ...
            default:
                if (arg.param_type == param_info::type::INTEGER || 
                    arg.raw_value < 1000) {
...
api_analyzer::api_analyzer(const analyzer_config& config)
    : pimpl(std::make_unique<impl>(config)) {}

api_analyzer::~api_analyzer() = default;

void api_analyzer::initialize(const util::module_range_index& module_index) {
    pimpl->initialize(module_index);
}

api_analysis_result api_analyzer::analyze_call(const api_context& ctx) {
    return pimpl->analyze_call(ctx);
}
...
const api_knowledge_db& api_analyzer::get_api_db() const {
    return pimpl->get_api_db();
}

api_analyzer::stats api_analyzer::get_stats() const {
    return pimpl->get_stats();
}

void api_analyzer::clear_caches() {
    pimpl->clear_caches();
}
