...
calling_convention_factory& calling_convention_factory::instance() {
    ...
    std::call_once(init_flag, [&]() {
        instance.register_platform_conventions();
    });
    return instance;
}
...
void calling_convention_factory::register_convention(
    ...
    std::function<calling_convention_ptr()> creator) {
    ...
    if (creators_.find(id) != creators_.end()) {
        log.wrn("overwriting existing convention registration", 
                redlog::field("id", to_string(id)));
    }
    
    creators_[id] = creator;
    name_to_id_[to_string(id)] = id;
    
    log.dbg("registered calling convention", 
            redlog::field("id", to_string(id)),
            redlog::field("total", creators_.size()));
}
...
calling_convention_ptr calling_convention_factory::create_default() const {
    return create(get_platform_default());
}
...
calling_convention_ptr calling_convention_factory::create_for_symbol(
    ...
    // platform-specific heuristics
        ...
        if (module_name.find("kernel32") != std::string::npos ||
            module_name.find("user32") != std::string::npos ||
...
void calling_convention_factory::register_platform_conventions() {
    ...
    log.inf("registered platform conventions", 
...
void calling_convention_factory::register_x86_conventions() {
    ...
    register_convention(
        ...
        []() { return std::make_shared<conventions::x86_cdecl>(); }
    );
...
void calling_convention_factory::register_x86_64_conventions() {
    ...
    register_convention(
        calling_convention_id::X86_64_SYSTEM_V,
...
void calling_convention_factory::register_arm64_conventions() {
    ...
    register_convention(
        calling_convention_id::AARCH64_AAPCS,
