...
std::vector<uint64_t> x86_64_system_v::extract_integer_args(
                ...
                if (float_reg_idx < max_float_reg_args) {
                    ...
                    for (int j = 0; j < 16; j++) {
                        arg.value.simd[j] = static_cast<uint8_t>(
                            ctx.read_stack(ctx.gpr->rsp + 8 + stack_offset + j) & 0xFF
                        );
                    }
                    arg.from_stack = true;
                    arg.stack_offset = stack_offset;
...
x86_64_system_v::register_info x86_64_system_v::get_register_info() const {
    return {
        ...
        .caller_saved_fpr = {"xmm0", "xmm1", "xmm2", "xmm3", "xmm4", "xmm5", "xmm6", "xmm7",
...
std::vector<double> x86_64_system_v::extract_float_args(
    const extraction_context& ctx,
    size_t count) const {
