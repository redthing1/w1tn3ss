...
x86_calling_convention::register_sources x86_thiscall::collect_registers(const extraction_context& ctx) const {
  register_sources regs{};
  regs.integer.push_back(ctx.gpr->ecx & 0xFFFFFFFFULL);
  return regs;
}

std::optional<x86_thiscall::variadic_info> x86_thiscall::get_variadic_info(const extraction_context&, size_t) const {
  return std::nullopt;
}
