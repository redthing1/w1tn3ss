...
void x86_cdecl::set_integer_args(QBDI::GPRState* gpr, const std::vector<uint64_t>& args,
                                std::function<void(uint64_t addr, uint64_t value)> stack_writer) const {
  throw std::runtime_error("x86 cdecl calling convention is not yet implemented for gadget execution. "
                          "Only x86-64 (System V and Microsoft) and ARM64 (AAPCS) are currently supported.");
}

void x86_cdecl::set_typed_args(QBDI::GPRState* gpr, QBDI::FPRState* fpr, const std::vector<typed_arg>& args,
                              std::function<void(uint64_t addr, uint64_t value)> stack_writer) const {
  throw std::runtime_error("x86 cdecl calling convention is not yet implemented for gadget execution. "
                          "Only x86-64 (System V and Microsoft) and ARM64 (AAPCS) are currently supported.");
}
