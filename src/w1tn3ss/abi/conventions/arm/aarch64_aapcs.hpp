...
public:
    ...
    // argument extraction
    std::vector<uint64_t> extract_integer_args(
        const extraction_context& ctx,
        size_t count) const override;
    
    std::vector<typed_arg> extract_typed_args(
        const extraction_context& ctx,
        const std::vector<arg_type>& types) const override;
    ...
    std::optional<variadic_info> get_variadic_info(
        const extraction_context& ctx,
        size_t fixed_arg_count) const override;
    ...
    stack_cleanup get_stack_cleanup() const override {
        return stack_cleanup::CALLER;
    }
    
    std::vector<double> extract_float_args(
        const extraction_context& ctx,
        size_t count) const override;
...
private:
    ...
    static constexpr std::array<size_t, 8> int_arg_regs = {
        offsetof(QBDI::GPRState, x0) / sizeof(QBDI::rword),
        offsetof(QBDI::GPRState, x1) / sizeof(QBDI::rword),
        offsetof(QBDI::GPRState, x2) / sizeof(QBDI::rword),
        offsetof(QBDI::GPRState, x3) / sizeof(QBDI::rword),
        offsetof(QBDI::GPRState, x4) / sizeof(QBDI::rword),
        offsetof(QBDI::GPRState, x5) / sizeof(QBDI::rword),
        offsetof(QBDI::GPRState, x6) / sizeof(QBDI::rword),
        offsetof(QBDI::GPRState, x7) / sizeof(QBDI::rword)
    };
