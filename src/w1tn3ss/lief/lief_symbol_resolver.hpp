...
public:
    ...
    std::optional<symbol_info> resolve(
        uint64_t address,
    ...
    std::optional<symbol_info> resolve_in_module(
...
private:
    ...
    std::optional<symbol_info> resolve_elf_symbol(
        LIEF::ELF::Binary* elf, uint64_t offset) const;
    
    std::optional<symbol_info> resolve_pe_symbol(
        LIEF::PE::Binary* pe, uint64_t offset) const;
    
    std::optional<symbol_info> resolve_macho_symbol(
...
public:
    ...
    void set_dyld_resolver(std::shared_ptr<macos_dyld_resolver> resolver) {
        dyld_resolver_ = resolver;
    }
#endif
...
private:
    ...
    mutable std::unordered_map<std::string, 
                              std::pair<std::list<std::string>::iterator,
                                       std::unique_ptr<LIEF::Binary>>> cache_;
...
public:
    ...
    std::optional<symbol_info> resolve(uint64_t, const util::module_range_index&) const {
        return std::nullopt;
    }
    
    std::optional<symbol_info> resolve_in_module(const std::string&, uint64_t) const {
        return std::nullopt;
    }
    
    std::vector<std::optional<symbol_info>> resolve_batch(
        const std::vector<uint64_t>&,
        const util::module_range_index&) const {
        return {};
    }
