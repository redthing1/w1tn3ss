...
macos_dyld_resolver::macos_dyld_resolver() 
    : log_("w1::lief::macos_dyld") {
    ...
    if (dump_dir_env && *dump_dir_env) {
        ...
        if (fs::exists(dump_dir_) && fs::is_directory(dump_dir_)) {
            log_.info("dyld shared cache dump directory configured",
                     redlog::field("path", dump_dir_));
        } else {
            log_.warn("dyld shared cache dump directory not found",
                     redlog::field("path", dump_dir_));
            dump_dir_.clear();
        }
    } else {
...
std::optional<std::string> macos_dyld_resolver::resolve_extracted_path(
    ...
    log_.dbg("attempting to resolve dyld cached library",
              redlog::field("original_path", original_path));
    
    // check if this is a system library that would be in dyld cache
    if (!is_dyld_cached_library(original_path)) {
        log_.dbg("not a dyld cached library",
                  redlog::field("path", original_path));
        return std::nullopt;
    }
    ...
    log_.ped("trying direct path mapping",
              redlog::field("mapped_path", direct_path));
    
    if (fs::exists(direct_path)) {
        log_.info("found library in dyld dump (direct mapping)",
                 redlog::field("original", original_path),
    ...
    log_.dbg("direct mapping failed, searching for library",
              redlog::field("library_name", library_name));
    
    if (auto found_path = find_library_in_dump(library_name)) {
        log_.info("found library in dyld dump (recursive search)",
                 redlog::field("original", original_path),
                 redlog::field("resolved", *found_path),
                 redlog::field("library_name", library_name));
        return found_path;
    }
    
    log_.dbg("library not found in dyld dump",
              redlog::field("original_path", original_path),
              redlog::field("library_name", library_name));
...
bool macos_dyld_resolver::is_dyld_cached_library(const std::string& path) const {
    log_.ped("checking if path is dyld cached library",
    ...
    static const std::vector<std::string> cached_lib_patterns = {
        "libsystem_",
        "libc++",
        "libobjc",
        "libdispatch",
    ...
    log_.ped("normalized path and extracted filename",
              redlog::field("norm_path", norm_path),
    ...
    for (const auto& prefix : cached_prefixes) {
        if (norm_path.find(prefix) == 0) {
            log_.ped("matched cached prefix",
                      redlog::field("prefix", prefix),
                      redlog::field("path", path));
            return true;
        }
    }
    ...
    for (const auto& pattern : cached_lib_patterns) {
        if (filename.find(pattern) == 0) {
            log_.ped("matched cached library pattern",
    ...
    log_.ped("not a dyld cached library",
              redlog::field("path", path));
    return false;
}

std::optional<std::string> macos_dyld_resolver::find_library_in_dump(
    const std::string& library_name) const {
    
    log_.dbg("searching for library in dump",
              redlog::field("library_name", library_name),
              redlog::field("dump_dir", dump_dir_));
    ...
    try {
        ...
        for (const auto& subdir : search_subdirs) {
            ...
            if (!fs::exists(search_path)) {
                log_.ped("search path does not exist",
                          redlog::field("search_path", search_path));
                continue;
            }
            
            log_.ped("searching in directory",
                      redlog::field("path", search_path),
                      redlog::field("library", library_name));
            ...
            for (const auto& entry : fs::directory_iterator(search_path)) {
                ...
                if (filename == library_name) {
                    log_.info("found exact library match",
                             redlog::field("library", library_name),
                ...
                if (dot_pos != std::string::npos) {
                    ...
                    if (filename.find(base_name) == 0 && 
                        filename.rfind(".dylib") == filename.length() - 6) {
                        
                        log_.dbg("found versioned match",
                                  redlog::field("requested", library_name),
                                  redlog::field("found", filename));
                        return entry.path().string();
                    }
                }
            }
        ...
        log_.dbg("library not found in common locations, trying recursive search",
                  redlog::field("library", library_name));
        ...
        log_.err("filesystem error while searching for library",
                redlog::field("library", library_name),
                redlog::field("error", e.what()));
        return std::nullopt;
    } catch (const std::exception& e) {
        log_.err("unexpected error while searching for library",
                redlog::field("library", library_name),
                redlog::field("error", e.what()));
        return std::nullopt;
    }
