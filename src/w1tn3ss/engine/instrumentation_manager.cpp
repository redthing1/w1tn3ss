...
bool instrumentation_manager::apply_instrumentation(QBDI::VM* vm) {
  ...
  log_.ped("configuring instrumentation");

  // always start fresh - remove all existing instrumentation
  log_.ped("removing all existing instrumented ranges");
  vm->removeAllInstrumentedRanges();

  // scan current modules
  log_.ped("scanning executable modules");
  util::module_scanner scanner;
  auto modules = scanner.scan_executable_modules();

  log_.ped("found executable modules", redlog::field("count", modules.size()));
...
bool instrumentation_manager::should_instrument_module(const util::module_info& module) const {
  ...
  if (name.starts_with("_unnamed_")) {
    log_.ped("excluding unnamed module", redlog::field("name", name));
    return false;
  }
  ...
  if (config_.use_default_conflicts && instrumentation_lists::is_conflict_module(name)) {
    log_.ped("excluding conflict module", redlog::field("name", name));
    return false;
  }
  ...
  if (!config_.force_exclude.empty() && instrumentation_lists::matches_any(name, config_.force_exclude)) {
    log_.ped("excluding force-excluded module", redlog::field("name", name));
    return false;
  }
  ...
  if (config_.use_default_criticals && instrumentation_lists::is_critical_module(name)) {
    log_.ped("including critical module", redlog::field("name", name));
    return true;
  }
  ...
  if (!config_.force_include.empty() && instrumentation_lists::matches_any(name, config_.force_include)) {
    log_.ped("including force-included module", redlog::field("name", name));
    return true;
  }
  ...
  if (!config_.module_filter.empty()) {
    ...
    if (matches) {
      log_.ped("including module (matches filter)", redlog::field("name", name));
    } else {
  ...
  if (module.is_system_library && !config_.include_system_modules) {
    log_.ped("excluding system module", redlog::field("name", name));
    return false;
  }

  // default: instrument
  log_.ped("including user module", redlog::field("name", name));
  return true;
}

bool instrumentation_manager::configure_instrumentation(QBDI::VM* vm, const std::vector<util::module_info>& modules) {
  log_.ped("applying instrumentation decisions");

  // log configuration summary at debug level
  if (!config_.module_filter.empty()) {
    log_.ped("using module filter", redlog::field("patterns", config_.module_filter));
  }
  log_.ped(
  ...
  log_.ped("instrumentation configured", redlog::field("added", added_count), redlog::field("skipped", skipped_count));
