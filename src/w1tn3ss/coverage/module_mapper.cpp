...
bool module_mapper::discover_process_modules() {
    ...
    if (success) {
        ...
        log_.info("module discovery completed",
                  redlog::field("regions", regions_.size()),
                  redlog::field("executable", executable_count),
                  redlog::field("user", user_count),
        ...
        if (!regions_.empty()) {
            ...
            if (min_addr != UINT64_MAX) {
                log_.verbose("executable memory layout",
                           redlog::field("min_addr", min_addr),
                           redlog::field("max_addr", max_addr),
                           redlog::field("space_size", max_addr - min_addr));
            }
        }
        
    } else {
        log_.error("module discovery failed - no memory regions discovered",
                  redlog::field("platform", 
#ifdef __APPLE__
                               "darwin"
#elif defined(__linux__)
                               "linux"
        ...
        log_.debug("diagnostic information",
                  redlog::field("pid", getpid()),
...
bool module_mapper::discover_qbdi_modules() {
    ...
    try {
        ...
        for (const auto& map : maps) {
            if (map.permission & QBDI::PF_EXEC) executable_maps++;
            if (map.permission & QBDI::PF_READ) readable_maps++;
            if (map.permission & QBDI::PF_WRITE) writable_maps++;
            
            if (!map.name.empty()) named_maps++;
            else anonymous_maps++;
        }
        
        log_.verbose("qbdi memory map summary",
                    redlog::field("total", maps.size()),
                    redlog::field("executable", executable_maps),
                    redlog::field("readable", readable_maps),
                    redlog::field("writable", writable_maps),
                    redlog::field("named", named_maps),
                    redlog::field("anonymous", anonymous_maps));
...
size_t module_mapper::register_discovered_modules() {
    ...
    for (const auto& region : regions_) {
        ...
        if (exclude_system_ && is_system_module(region.name)) {
            excluded_count++;
            log_.trace("excluding system module",
        ...
        if (!matches_target_pattern(region.name)) {
            ...
            log_.trace("module filtered by target patterns",
                      redlog::field("name", region.name),
                      redlog::field("patterns", target_patterns_.size()));
            continue;
        }
        ...
        if (module_id != UINT16_MAX) {
            ...
            log_.warn("failed to register module",
                     redlog::field("name", region.name),
                     redlog::field("start", region.start),
                     redlog::field("size", region.size()));
        }
    ...
    log_.info("module registration completed",
              ...
              redlog::field("total", regions_.size()),
              redlog::field("excluded", excluded_count),
              redlog::field("not_exec", non_executable_count),
              redlog::field("filtered", pattern_filtered_count));
    
    if (registered_count == 0 && regions_.size() > 0) {
        log_.warn("no modules were registered despite having memory regions",
                 redlog::field("executable", get_executable_count()),
...
bool module_mapper::discover_modules_darwin() {
    ...
    if (!success) {
        log_.warn("qbdi module discovery failed on darwin",
                 redlog::field("pid", getpid()));
    }
...
bool module_mapper::discover_modules_linux() {
    ...
    if (!maps_file) {
        ...
        log_.error("could not open /proc/self/maps",
                  redlog::field("errno", open_errno),
    ...
    while (std::getline(maps_file, line)) {
        ...
        if (!(iss >> address_range >> perms >> offset >> dev >> inode)) {
            parse_errors++;
            log_.trace("failed to parse maps line",
                      redlog::field("line", line_count),
                      redlog::field("content", line));
            continue;
        }
        ...
        if (dash_pos == std::string::npos) {
            parse_errors++;
            log_.trace("invalid address range format",
                      redlog::field("line", line_count),
                      redlog::field("range", address_range));
            continue;
        }
        ...
        try {
            ...
            log_.trace("parsed memory region",
                      redlog::field("start", start),
                      redlog::field("end", end),
                      redlog::field("perms", perms),
                      redlog::field("name", pathname),
                      redlog::field("executable", is_executable));
            
        } catch (const std::exception& e) {
            parse_errors++;
            log_.warn("failed to parse address range",
                     redlog::field("line", line_count),
                     redlog::field("range", address_range),
                     redlog::field("error", e.what()));
        }
    ...
    log_.info("linux module discovery completed",
              redlog::field("lines", line_count),
              redlog::field("regions", parsed_count),
    ...
    for (const auto& map : maps) {
        try {
            ...
            if (start_addr >= end_addr) {
                log_.warn("invalid memory range detected",
                         redlog::field("start", start_addr),
                         redlog::field("end", end_addr),
            ...
            log_.trace("converted memory map",
                      redlog::field("start", start_addr),
                      redlog::field("end", end_addr),
                      redlog::field("size", size),
                      redlog::field("perms", perm_str),
                      redlog::field("name", name),
                      redlog::field("executable", is_executable));
            
        } catch (const std::exception& e) {
            log_.warn("failed to convert memory map",
                     redlog::field("error", e.what()));
            skipped_count++;
        }
    }
    
    log_.debug("qbdi memory map conversion completed",
               redlog::field("input", maps.size()),
               redlog::field("converted", converted_count),
               redlog::field("skipped", skipped_count),
               redlog::field("total_bytes", total_memory),
               redlog::field("exec_bytes", executable_memory));
