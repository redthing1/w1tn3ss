...
public:
  ...
  impl() : log(redlog::get_logger("w1tn3ss.w1cov_standalone")), initialized(false) {
...
bool w1cov_standalone::instrument_function(void* func_ptr, const std::string& name) {
  ...
  pimpl->log.info(
      "marking function for instrumentation", redlog::field("name", name),
...
bool w1cov_standalone::call_instrumented_function(void* func_ptr, const std::vector<uint64_t>& args, uint64_t* result) {
  if (!pimpl->initialized) {
    pimpl->log.error("not initialized");
    return false;
  }
  ...
  pimpl->log.info(
      "calling instrumented function", redlog::field("address", reinterpret_cast<uint64_t>(func_ptr)),
  ...
  try {
    ...
    pimpl->log.info(
        "instrumented function call completed", redlog::field("return_value", retvalue),
...
size_t w1cov_standalone::get_coverage_count() const { return pimpl->covered_addresses.size(); }

bool w1cov_standalone::run_instrumented_binary(
    const std::string& binary_path, const std::vector<std::string>& args, int* exit_code
) {
  if (!pimpl->initialized) {
    pimpl->log.error("not initialized");
    return false;
  ...
  pimpl->log.info(
      "running instrumented binary", redlog::field("binary_path", binary_path), redlog::field("arg_count", args.size())
  );
...
bool w1cov_standalone::export_coverage(const std::string& output_file) {
  ...
  if (pimpl->covered_addresses.size() > 0) {
    ...
    if (module_id != UINT16_MAX) {
      ...
      pimpl->log.info(
          "created temporary module for export", redlog::field("module_id", module_id),
          redlog::field("base", module_base), redlog::field("end", module_end),
          redlog::field("addresses", pimpl->covered_addresses.size())
      );
    }
  }
  ...
  if (success) {
    pimpl->log.info(
        "coverage export successful", redlog::field("output_file", output_file),
...
void w1cov_standalone::print_stats() const {
  pimpl->log.info(
      "coverage statistics", redlog::field("unique", pimpl->covered_addresses.size()),
