...
void configure_from_env() {
  ...
  if (g_debug_mode) {
    printf(
        "[W1COV_RT] Configuration: enabled=%d, output=%s, debug=%d\n", g_enabled, get_output_file()->c_str(),
        g_debug_mode
    );
    fflush(stdout);
  }
...
QBDI::VMAction coverage_callback(
  ...
  if (instAnalysis) {
    ...
    if (g_debug_mode && g_instruction_count % 10000 == 0) {
      printf(
          "[W1COV_RT] Traced %llu instructions, %zu unique addresses\n", g_instruction_count, covered_addresses->size()
      );
      fflush(stdout);
    }
  }
...
bool export_drcov_coverage() {
  ...
  try {
    ...
    auto builder = drcov::builder().set_flavor("w1cov_runtime").set_module_version(drcov::module_table_version::v2);
    ...
    printf(
        "[W1COV_RT] Coverage exported: %zu addresses, %zu modules -> %s\n", covered_addresses->size(),
