...
uint16_t coverage_collector::add_module(const std::string& path, uint64_t base, uint64_t end, uint64_t entry) {
  ...
  for (const auto& module : modules_) {
    if (module.path == path && module.base_address == base) {
      log_.verbose("module already exists", redlog::field("path", path), redlog::field("id", module.id));
      return module.id;
    }
  }
...
drcov::coverage_data coverage_collector::export_drcov_data() const {
  ...
  for (const auto& module : modules_) {
    try {
      ...
      if (module.base_address >= module.end_address) {
        log_.warn(
            "invalid module address range detected", redlog::field("id", module.id), redlog::field("path", module.path),
      ...
      if (module.path.empty()) {
        log_.warn("module has empty path", redlog::field("id", module.id), redlog::field("base", module.base_address));
        invalid_modules++;
        continue;
      }
      ...
      log_.error(
          "failed to add module to drcov builder", redlog::field("id", module.id), redlog::field("path", module.path),
  ...
  for (const auto& block : basic_blocks_) {
    try {
      ...
      if (!module) {
        log_.trace(
            "basic block references unknown module", redlog::field("id", block.module_id),
      ...
      if (block.address < module->base_address || block.address >= module->end_address) {
        log_.warn(
            "basic block address outside module bounds", redlog::field("id", block.module_id),
            redlog::field("address", block.address), redlog::field("base", module->base_address),
      ...
      if (offset >= (module->end_address - module->base_address)) {
        log_.warn(
            "calculated offset exceeds module size", redlog::field("id", block.module_id),
            redlog::field("offset", offset), redlog::field("size", module->end_address - module->base_address)
        );
        invalid_blocks++;
        continue;
      }
      ...
      log_.error(
          "failed to add basic block to drcov builder", redlog::field("id", block.module_id),
          redlog::field("address", block.address), redlog::field("error", e.what())
      );
      invalid_blocks++;
    }
  }
  ...
  try {
    ...
    log_.error("failed to build drcov data structure", redlog::field("error", e.what()));
...
bool coverage_collector::write_drcov_file(const std::string& filepath) const {
  ...
  if (!parent_dir.empty()) {
    ...
    if (!std::filesystem::exists(parent_dir, fs_error)) {
      log_.warn(
          "output directory does not exist, attempting to create", redlog::field("directory", parent_dir.string())
      );

      if (!std::filesystem::create_directories(parent_dir, fs_error)) {
        log_.error(
            "failed to create output directory", redlog::field("directory", parent_dir.string()),
            redlog::field("error", fs_error.message())
        );
        return false;
      }

      log_.debug("created output directory", redlog::field("directory", parent_dir.string()));
    }
    ...
    if (fs_error) {
      log_.error(
          "failed to check output directory status", redlog::field("directory", parent_dir.string()),
          redlog::field("error", fs_error.message())
      );
      return false;
    }
    ...
    if (!std::filesystem::is_directory(dir_status)) {
      log_.error("output path parent is not a directory", redlog::field("path", parent_dir.string()));
      return false;
    }
  }
  ...
  if (fs_error) {
    log_.warn(
        "failed to check if output file exists", redlog::field("filepath", filepath),
        redlog::field("error", fs_error.message())
    );
  } else if (file_exists) {
    auto file_size = std::filesystem::file_size(filepath, fs_error);
    log_.debug(
        "output file already exists, will be overwritten", redlog::field("filepath", filepath),
  ...
  try {
    ...
    log_.debug("writing drcov data to file", redlog::field("filepath", filepath));

    drcov::write(filepath, coverage_data);

    // verify file was written successfully
    if (!std::filesystem::exists(filepath, fs_error)) {
      log_.error("drcov file was not created", redlog::field("filepath", filepath));
      return false;
    }
    ...
    if (fs_error) {
      log_.warn(
          "failed to get final file size", redlog::field("filepath", filepath),
          redlog::field("error", fs_error.message())
      );
    }
    ...
    log_.error(
        "failed to write drcov file", redlog::field("filepath", filepath), redlog::field("error", e.what()),
    ...
    if (std::filesystem::exists(filepath, cleanup_error) && !cleanup_error) {
      if (std::filesystem::remove(filepath, cleanup_error)) {
        log_.debug("removed incomplete drcov file", redlog::field("filepath", filepath));
      } else {
        log_.warn(
            "failed to remove incomplete drcov file", redlog::field("filepath", filepath),
