...
struct basic_block_info {
    ...
    basic_block_info(uint64_t addr, uint16_t sz, uint16_t mod_id)
        : address(addr), size(sz), module_id(mod_id) {}
};
...
struct module_info {
    ...
    module_info(uint16_t module_id, const std::string& module_path,
                uint64_t base, uint64_t end, uint64_t entry = 0)
        : id(module_id), path(module_path), base_address(base), 
          end_address(end), entry_point(entry) {}
