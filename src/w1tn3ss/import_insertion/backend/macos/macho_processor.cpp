...
template<typename T>
T MachOProcessor::peek_at(std::streamoff pos) {
    T result;
...
bool MachOProcessor::check_load_commands(struct mach_header* mh, size_t header_offset, 
    ...
    log_processor.trc("checking load commands", redlog::field("count", ncmds), redlog::field("commands_offset", commands_offset));
    ...
    for (uint32_t i = 0; i < ncmds; i++) {
        ...
        log_processor.dbg("processing load command", redlog::field("index", i), redlog::field("cmd", cmd), redlog::field("size", cmdsize));
        ...
        switch (cmd) {
            ...
            case LC_LOAD_WEAK_DYLIB: {
                ...
                log_processor.dbg("found existing dylib load command", redlog::field("path", existing_path), redlog::field("weak", cmd == LC_LOAD_WEAK_DYLIB));
            ...
            case LC_SEGMENT_64:
                ...
                if (cmd == LC_SEGMENT) {
                    ...
                    log_processor.dbg("found 32-bit segment", redlog::field("name", std::string(seg_cmd.segname, strnlen(seg_cmd.segname, 16))));
                    if (strncmp(seg_cmd.segname, "__LINKEDIT", 16) == 0) {
                        log_processor.trc("found __LINKEDIT segment (32-bit)", redlog::field("fileoff", SWAP32(seg_cmd.fileoff, mh->magic)), redlog::field("filesize", SWAP32(seg_cmd.filesize, mh->magic)));
                    ...
                    file_.read(reinterpret_cast<char*>(&seg_cmd), sizeof(seg_cmd));
                    log_processor.dbg("found 64-bit segment", redlog::field("name", std::string(seg_cmd.segname, strnlen(seg_cmd.segname, 16))));
                    if (strncmp(seg_cmd.segname, "__LINKEDIT", 16) == 0) {
                        log_processor.trc("found __LINKEDIT segment (64-bit)", redlog::field("fileoff", SWAP64(seg_cmd.fileoff, mh->magic)), redlog::field("filesize", SWAP64(seg_cmd.filesize, mh->magic)));
...
bool MachOProcessor::process_fat_binary(const std::string& dylib_path, uint32_t magic) {
    ...
    log_processor.trc("fat binary details", redlog::field("magic", magic), redlog::field("little_endian", IS_LITTLE_ENDIAN(magic)));
    ...
    for (uint32_t i = 0; i < nfat_arch; i++) {
        ...
        log_processor.trc("processing architecture", redlog::field("index", i), redlog::field("offset", offset), redlog::field("size", slice_size), redlog::field("cputype", SWAP32(archs[i].cputype, magic)));
        ...
        if (!process_mach_o(dylib_path, offset, &slice_size)) {
            ...
            log_processor.dbg("architecture processed successfully", redlog::field("index", i), redlog::field("new_size", slice_size));
        }
    ...
    if (failures == 0) {
        ...
        log_processor.warn("added dylib load command to some architectures", redlog::field("successful", nfat_arch - failures), redlog::field("total", nfat_arch));
        return true;
    }
}

bool MachOProcessor::process_mach_o(const std::string& dylib_path, std::streamoff header_offset, 
                   std::streamoff* slice_size) {
    file_.seekg(header_offset);
    struct mach_header mh;
    file_.read(reinterpret_cast<char*>(&mh), sizeof(mh));

    log_processor.trc("processing mach-o binary", redlog::field("header_offset", header_offset), redlog::field("magic", mh.magic), redlog::field("is_64bit", IS_64_BIT(mh.magic)));
    ...
    log_processor.dbg("mach-o header parsed", redlog::field("ncmds", SWAP32(mh.ncmds, mh.magic)), redlog::field("sizeofcmds", SWAP32(mh.sizeofcmds, mh.magic)), redlog::field("commands_offset", commands_offset));
    ...
    log_processor.trc("creating dylib load command", redlog::field("dylib_path", dylib_path), redlog::field("weak", weak_flag_), redlog::field("cmdsize", cmdsize), redlog::field("padded_path_size", dylib_path_size));
    ...
    log_processor.dbg("space check at insertion point", redlog::field("position", insert_pos), redlog::field("required_size", cmdsize), redlog::field("has_space", has_space));
    ...
    log_processor.dbg("wrote dylib path", redlog::field("original_length", dylib_path.length()), redlog::field("padded_length", dylib_path_size));
    ...
    log_processor.trc("updating mach-o header", redlog::field("old_ncmds", SWAP32(mh.ncmds, mh.magic) - 1), redlog::field("new_ncmds", new_ncmds), redlog::field("new_sizeofcmds", new_sizeofcmds));
