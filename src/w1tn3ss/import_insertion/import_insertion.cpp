...
error_code validate_config(const config& cfg) {
    ...
    if (cfg.library_path[0] != '@' && !fs::exists(cfg.library_path)) {
        if (!cfg.assume_yes) {
            ...
            log.debug("library path does not exist but assume_yes=true", 
                     redlog::field("path", cfg.library_path));
        }
    }
...
result insert_library_import(const config& cfg) {
    ...
    log.info("starting library import insertion", 
            redlog::field("library_path", cfg.library_path),
            redlog::field("target_binary", cfg.target_binary),
            redlog::field("weak_import", cfg.weak_import));
    ...
    log.debug("performing platform-specific import insertion", 
             redlog::field("platform", platform_str));
...
#else
    log.error("platform not supported for library import insertion", 
             redlog::field("platform", platform_str));
    insertion_result = result{
        .code = error_code::platform_not_supported,
        .error_message = "platform not supported for library import insertion"
    };
#endif
    ...
    if (insertion_result.success()) {
        ...
        log.error("library import insertion failed", 
                 redlog::field("error_code", static_cast<int>(insertion_result.code)),
...
std::string error_code_to_string(error_code code) {
    switch (code) {
        ...
        case error_code::file_access_denied: return "file_access_denied";
        case error_code::invalid_output_path: return "invalid_output_path";
        case error_code::output_already_exists: return "output_already_exists";
        case error_code::invalid_binary_format: return "invalid_binary_format";
        case error_code::unknown_magic_number: return "unknown_magic_number";
        case error_code::unsupported_architecture: return "unsupported_architecture";
        case error_code::corrupted_header: return "corrupted_header";
        case error_code::duplicate_library: return "duplicate_library";
        case error_code::library_path_invalid: return "library_path_invalid";
        case error_code::insufficient_space: return "insufficient_space";
        case error_code::code_signature_conflict: return "code_signature_conflict";
        case error_code::code_signature_removal_failed: return "code_signature_removal_failed";
        case error_code::user_declined: return "user_declined";
        case error_code::interactive_prompt_failed: return "interactive_prompt_failed";
        case error_code::platform_not_supported: return "platform_not_supported";
        case error_code::insufficient_privileges: return "insufficient_privileges";
        case error_code::out_of_memory: return "out_of_memory";
        case error_code::system_error: return "system_error";
        case error_code::configuration_invalid: return "configuration_invalid";
...
#ifdef __APPLE__
    return macos::check_import_capabilities();
#elif defined(__linux__)
    return linux_impl::check_import_capabilities();
#elif defined(_WIN32)
    return windows::check_import_capabilities();
#else
    return false;
#endif
}
