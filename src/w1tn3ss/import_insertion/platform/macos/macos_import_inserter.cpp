...
result insert_library_import(const config& cfg) {
    ...
    if (cfg.library_path[0] != '@' && !fs::exists(cfg.library_path)) {
        ...
        if (!cfg.assume_yes) {
            return result{
                .code = error_code::library_path_invalid,
                .error_message = "library path does not exist: " + cfg.library_path
            };
        } else {
    ...
    if (!cfg.in_place) {
        ...
        if (copyfile(cfg.target_binary.c_str(), final_binary_path.c_str(), nullptr, 
                    COPYFILE_DATA | COPYFILE_UNLINK) != 0) {
            log.error("failed to copy file", 
                     redlog::field("source", cfg.target_binary), 
    ...
    try {
        ...
        backend::macos::MachOProcessor processor(final_binary_path, cfg.weak_import, cfg.strip_code_signature, 
                               !cfg.assume_yes);
        ...
        if (processor.insert_dylib_load_command(cfg.library_path)) {
            ...
            log.inf("successfully added dylib load command", 
                   redlog::field("dylib", cfg.library_path), 
                   redlog::field("binary", final_binary_path), 
                   redlog::field("load_command", lc_name));
            return result{
                .code = error_code::success,
                .error_message = "library import inserted successfully"
            };
        } else {
            ...
            return result{
                ...
                .error_message = "mach-o processor failed to insert library import"
            };
        }
        ...
        return result{
            .code = error_code::system_error,
            .error_message = std::string("binary processing error: ") + e.what()
        };
    }
}
