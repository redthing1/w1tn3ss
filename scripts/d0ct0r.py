...
class FileOperations:
    ...
    def copy_file(src: Path, dst: Path, use_elevation: bool = False) -> bool:
        ...
        if not use_elevation:
            try:
                ...
                if platform.system() == "Windows":
                    console.print("[red]permission denied - please run as administrator[/red]")
                    return False
                else:
                    console.print("[yellow]permission denied - retrying with sudo[/yellow]")
        ...
        if platform.system() == "Windows":
            console.print("[red]windows elevation required - please run as administrator[/red]")
            return False
        else:
            ...
            if not dst_dir.exists():
                ...
                try:
                    ...
                    if mkdir_result.returncode != 0:
                        console.print(f"[red]failed to create directory with sudo: {mkdir_result.stderr}[/red]")
                        return False
                except Exception as e:
    ...
    @staticmethod
    def set_permissions(file_path: Path, permissions: int, use_elevation: bool = False) -> bool:
        ...
        if not use_elevation:
            try:
                ...
                console.print(f"[green]✓ restored permissions: {oct(permissions)}[/green]")
                return True
            except PermissionError:
                if platform.system() == "Windows":
                    console.print("[yellow]windows permission model differs - skipping[/yellow]")
        ...
        if platform.system() == "Windows":
            console.print("[yellow]windows elevation for permissions not implemented[/yellow]")
            return True  # windows permission model is different
        else:
            ...
            try:
                ...
                result = subprocess.run(cmd, capture_output=True, text=True)
                if result.returncode == 0:
                    console.print(f"[green]✓ restored permissions with sudo: {oct(permissions)}[/green]")
                    return True
                else:
...
class BackupManager:
    ...
    @staticmethod
    def create_backup(file_path: Path, force: bool = False, use_elevation: bool = False) -> Path:
        """
        ...
        console.print(f"[blue]creating backup: {file_path.name} -> {backup_path.name}[/blue]")
    ...
    def run_p1llx(cmd_args: List[str], sudo: bool = False) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                console.print(f"[red]p1llx failed with exit code: {result.returncode}[/red]")
                return False
...
def main_callback(
    ...
    verbose: int = typer.Option(0, "-v", "--verbose", count=True, help="increase verbosity (-v, -vv, -vvv)"),
):
...
def auto_cure(
    ...
    platform_override: Optional[str] = typer.Option(None, "-p", "--platform", help="platform override (e.g., darwin:arm64)"),
    no_backup: bool = typer.Option(False, "--no-backup", help="disable backup creation"),
):
    """
    ...
    console.print(Panel(f"[bold]d0ct0r auto-cure[/bold]\n"
    ...
    if not no_backup and BackupManager.should_use_backup(input_path, output_path):
        backup_path = BackupManager.create_backup(input_path, force=False, use_elevation=global_sudo)
    ...
    if output_path:
        ...
        if actual_input != input_path:  # we're using a backup
            cmd_args.extend(["-o", str(input_path)])  # output to original file, preserve backup
    ...
    # restore permissions
    if not PermissionManager.set_permissions(target_file, original_permissions, global_sudo):
...
def cure(
    ...
    input_file: str = typer.Option(..., "-i", "--input", help="input file path"),
    output_file: Optional[str] = typer.Option(None, "-o", "--output", help="output file path"),
    platform_override: Optional[str] = typer.Option(None, "-p", "--platform", help="platform override"),
):
    """
...
def patch(
    ...
    """
    cmd_args = ["patch", f"--address={address}", f"--replace={replace}", "-i", input_file]
...
def poison(
    ...
    process_name: Optional[str] = typer.Option(None, "--process-name", help="target process name"),
    suspended: bool = typer.Option(False, "--suspended", help="start in suspended mode"),
    args: Optional[List[str]] = typer.Argument(None, help="target binary and arguments"),
):
    """
    ...
    if spawn:
        ...
        console.print("[red]must specify target: --spawn, --pid, or --process-name[/red]")
        raise typer.Exit(1)
