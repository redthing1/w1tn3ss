...
int test_w1cov(int verbose_level = 0) {
  ...
  if (!session.trace_function((void*) test_coverage_control_flow, {5}, &result1)) {
    std::cout << "failed to trace function (value < 10)\n";
    return 1;
  }

  if (!session.trace_function((void*) test_coverage_control_flow, {15}, &result2)) {
    std::cout << "failed to trace function (10 <= value < 20)\n";
    return 1;
  }

  if (!session.trace_function((void*) test_coverage_control_flow, {30}, &result3)) {
    std::cout << "failed to trace function (20 <= value < 50)\n";
    return 1;
  }

  if (!session.trace_function((void*) test_coverage_control_flow, {100}, &result4)) {
...
int test_w1xfer(int verbose_level = 0) {
  ...
  uint64_t result;
  if (!session.trace_function((void*) test_xfer_library_calls, {1024}, &result)) {
